...
public class AuthService {
     ...
     * @return LoginResponse con token JWT, tipo bearer y datos del usuario
     *         autenticado
     * @throws BusinessException si usuario no existe, está inactivo, o credenciales
     *                           son inválidas
     */
    @Transactional
    public LoginResponse login(String username, String password) {
        try {
            ...
            // Se usa try-catch porque la relación puede no estar inicializada (lazy
            // loading)
            ...
            // El token incluye username y autoridades. Expiración configurada en
            // JwtTokenProvider
     ...
     * 2. Valida que email no exista (unicidad requerida para recuperación de
     * contraseña)
     ...
     * 
     * @param request DTO con datos del usuario (username, email, password, nombre,
     *                apellido, rol)
     * @return Usuario creado (sin exponer contraseña)
     * @throws BusinessException si validaciones fallan (username/email duplicados,
     *                           rol no existe, contraseña débil)
     */
    @Transactional
    public Usuario register(RegisterRequest request) {
        ...
        if (usuarioRepository.existsByUsername(Objects.requireNonNull(username))) {
        ...
        if (usuarioRepository.existsByCorreo(Objects.requireNonNull(email))) {
        ...
        // VALIDACIÓN 3: Validar que la contraseña cumpla políticas (longitud,
        // complejidad, etc)
        ...
        Rol rol = rolRepository.findByNombreRol(Objects.requireNonNull(nombreRol))
                .orElseThrow(() -> {
        ...
        Usuario usuarioRegistrado = usuarioRepository.save(Objects.requireNonNull(usuario));
    ...
    public String forgotPassword(ForgotPasswordRequest request) {
        ...
        Usuario usuario = usuarioRepository.findByCorreo(Objects.requireNonNull(emailOrUsername))
    ...
    public void resetPassword(ResetPasswordRequest request) {
        ...
        Usuario usuario = usuarioRepository.findByPasswordResetToken(Objects.requireNonNull(token))
                .orElseThrow(() -> {
        ...
        // PASO 3: Validar que la nueva contraseña cumpla políticas (longitud,
        // complejidad, etc)
